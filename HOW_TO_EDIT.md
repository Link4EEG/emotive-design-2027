...
- **Research Films** — 왼쪽 플레이어 하나 + 오른쪽 번호 재생목록(Take 01–08). 목록에서 고르면 바로 재생되고, 한 편이 끝나면 다음 편이 이어서 재생됩니다(마지막 편에서 멈춤).
  - 영상 추가/순서 변경: 웹용 파일을 `assets/films/`에, 포스터를 `assets/films/posters/`에 넣고 `index.html`의 `RESEARCH_FILMS` 배열에 한 줄을 추가합니다(제목 `title`, 출처 `meta`, 상영 시간 `time`).
  - 웹용 파일 만들기(화질 그대로, 메타데이터 제거, 바로 재생되게): `ffmpeg -i 원본.mp4 -c copy -map_metadata -1 -movflags +faststart assets/films/이름.mp4`
  - 예전에 혼자 걸려 있던 Monster Space 트레일러는 언제나 **맨 끝(Take 08)** 입니다. 편집 모드에서 영상 위 **Change the last film** 버튼으로 교체할 수 있습니다.
